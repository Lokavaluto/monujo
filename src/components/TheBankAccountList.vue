<template>
  <div class="accounts card custom-card custom-card-padding">
    ...
    >
      ...
      <span
        v-if="
          hasCreditRequestValidationRights && hasUserAccountValidationRights
        "
      >
        <a
          @click="$modal.open('AdminModal', {})"
          class="button is-default is-rounded refresh is-pulled-right mr-3"
        >
          <span :class="{ hide: accountsLoading || isAccountsLoadingRetrying }">
            {{ $gettext("Admin") }}
          </span>
